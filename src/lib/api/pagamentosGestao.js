...
export async function getIntegracaoStripe(token) {
  if (!token) return { error: "Token em falta." };
  try {
    const response = await fetch(`${getApiUrl()}/pagamentos/stripe/integracao/meu/`, {
      headers: { Authorization: `Token ${token}` },
    });
    if (!response.ok) {
      throw new Error(await parseError(response));
    ...
    console.error("API Error (getIntegracaoStripe):", error);
    return { error: error.message };
  }
}

export async function iniciarOAuthStripe(token) {
  if (!token) return { error: "Token em falta." };
  try {
    const response = await fetch(`${getApiUrl()}/pagamentos/stripe/oauth/iniciar/`, {
      headers: { Authorization: `Token ${token}` },
    });
    if (!response.ok) {
      throw new Error(await parseError(response));
    ...
    console.error("API Error (iniciarOAuthStripe):", error);
    return { error: error.message };
  }
}

export async function desconectarStripe(token) {
  if (!token) return { error: "Token em falta." };
  try {
    const response = await fetch(
      `${getApiUrl()}/pagamentos/stripe/integracao/desconectar/`,
      {
        method: "POST",
        headers: { Authorization: `Token ${token}` },
      }
    ...
    console.error("API Error (desconectarStripe):", error);
    return { error: error.message };
  }
}
