  ...
  return (
    <div className="space-y-8 mt-6">
      {categorias.map((categoria) => {
        const itemsInCategory = itensFiltrados.filter(item => item.categoria === categoria.nome);
        if (itemsInCategory.length === 0 && searchTerm.length > 0) return null;

        return (
          <div key={categoria.id} className="bg-white p-6 rounded-lg shadow-md">
            <div className="flex justify-between items-center mb-4 pb-4 border-b border-gray-300">
              <h2 className="text-xl font-semibold text-gray-700">{categoria.nome}</h2>
              <div className="space-x-3">
                <button onClick={() => onEditCategory(categoria)} className="text-sm text-indigo-600 hover:underline">Editar</button>
                <button onClick={() => onDeleteCategory(categoria)} className="text-sm text-red-600 hover:underline">Apagar</button>
              </div>
            </div>
                ...
                <li key={item.id} className="py-3 flex justify-between items-center">
                  <div className="flex items-center gap-4">
                    <input
                      ...
                      onChange={() => onToggleDisponibilidade(item)}
                    />
                    <span className={`font-medium ${item.disponivel ? 'text-gray-800' : 'text-gray-400 line-through'}`}>
                      {item.nome}
                    </span>
                  </div>
                  <div className="space-x-3">
                    <button onClick={() => onEditItem(item)} className="text-sm text-indigo-600 hover:underline">Editar</button>
                    <button onClick={() => onDeleteItem(item)} className="text-sm text-red-600 hover:underline">Apagar</button>
                  </div>
                </li>
              ))}
              {itemsInCategory.length === 0 && (
                <li className="py-3 text-sm text-gray-500">Nenhum item nesta categoria.</li>
              )}
            </ul>
          </div>
        );
...
const GerirMesas = ({ mesas, restaurante }) => { // O token já não é mais necessário como prop
  const [isZipping, setIsZipping] = useState(false);
  const API_URL = process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8001/api";

  const handleDownloadUnico = async (mesaId, numeroMesa) => {
    // --- ALTERAÇÃO PRINCIPAL AQUI ---
    // Lemos o token diretamente do localStorage no momento do clique.
    ...
    const url = `${API_URL}/mesas/${mesaId}/baixar-qrcode/`;
    try {
      const response = await fetch(url, {
        headers: { 'Authorization': `Token ${token}` }
      });
      if (!response.ok) throw new Error(`Falha ao baixar a imagem (Status: ${response.status})`);
      
      const blob = await response.blob();
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `qrcode-mesa-${numeroMesa}.png`;
      ...
      alert("Não foi possível baixar o QR code. Verifique a consola para mais detalhes.");
    }
  };

  const handleDownloadTodos = async () => {
    const token = localStorage.getItem("authToken"); // Também lemos aqui
    if (!token) {
      alert("Erro de autenticação. Por favor, faça login novamente.");
      return;
    }
    ...
    try {
      await Promise.all(mesas.map(async (mesa) => {
        const url = `${API_URL}/mesas/${mesa.id}/baixar-qrcode/`;
        const response = await fetch(url, {
          headers: { 'Authorization': `Token ${token}` }
        });
        if (response.ok) {
          const blob = await response.blob();
          zip.file(`qrcode-mesa-${mesa.numero}.png`, blob);
        }
      }));
      ...
      link.download = `qrcodes-${restaurante?.slug || 'mesas'}.zip`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(link.href);
    } catch (error) {
      console.error("Erro ao gerar o ficheiro .zip:", error);
      alert("Ocorreu um erro ao gerar o ficheiro .zip.");
    } finally {
      setIsZipping(false);
    }
  };
  ...
  return (
    <div className="mt-6 bg-white p-6 rounded-lg shadow-md">
      <div className="flex justify-between items-center mb-4 pb-4 border-b border-gray-300">
        <h2 className="text-xl font-semibold text-gray-700">Mesas do Restaurante</h2>
        {mesas.length > 0 && (
          <button 
            onClick={handleDownloadTodos}
            disabled={isZipping}
            className="flex items-center gap-2 bg-gray-700 text-white font-bold py-2 px-4 rounded-lg hover:bg-gray-800 disabled:bg-gray-400"
          >
            <DownloadIcon />
            {isZipping ? 'A gerar...' : 'Baixar Todos'}
          </button>
        )}
      </div>
      {mesas.length > 0 ? (
        <ul className="divide-y divide-gray-200">
          {mesas.map(mesa => (
            <li key={mesa.id} className="py-3 flex justify-between items-center">
              <span className="font-medium text-gray-800">Mesa {mesa.numero}</span>
              <button 
                onClick={() => handleDownloadUnico(mesa.id, mesa.numero)} 
        ...
        <p className="text-gray-500 py-3">Ainda não há mesas configuradas.</p>
      )}
    </div>
  );
};
...
const GerirRestaurante = ({ restaurante, onSave }) => {
  const [formData, setFormData] = useState({
    nome: '', cor_principal: '#4F46E5', quantidade_mesas: 0
  });
          ...
          <div>
            ...
            <div className="mt-1 flex items-center space-x-4">
              {logoPreview ? 
                ...
                <div className="h-16 w-16 rounded-full bg-gray-100 flex items-center justify-center text-gray-400">?</div>
              }
...
export default function ConfiguracoesPage() {
  ...
  const handleItemFormSubmit = async (itemData) => {
    const token = localStorage.getItem("authToken");
    if (!token) return { success: false, error: "Autenticação em falta." };
    ...
    try {
      const result = itemToEdit 
        ? await updateMenuItem(token, itemToEdit.id, formData) 
      ...
      if (result && !result.error) {
        ...
        return { success: false, error: result?.error || "Erro desconhecido da API." };
      }
    } catch (e) {
      console.error("Falha crítica ao submeter o formulário do item:", e);
      return { success: false, error: "Ocorreu uma falha na comunicação com o servidor." };
    }
  };
  ...
  return (
    <div>
      ...
      {activeTab === 'mesas' && <GerirMesas mesas={mesas} />}
