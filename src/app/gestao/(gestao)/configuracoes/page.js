...
const GerirRestaurante = ({ restaurante, onSave }) => {
  const [formData, setFormData] = useState({ nome: '', cor_principal: '#4F46E5', quantidade_mesas: 0 }); const [logoFile, setLogoFile] = useState(null); const [logoPreview, setLogoPreview] = useState(''); const [isSaving, setIsSaving] = useState(false); useEffect(() => { if (restaurante) { setFormData({ nome: restaurante.nome || '', cor_principal: restaurante.cor_principal || '#4F46E5', quantidade_mesas: restaurante.quantidade_mesas || 0, }); setLogoPreview(restaurante.logo || ''); } }, [restaurante]); const handleChange = (e) => { const { name, value } = e.target; setFormData(prev => ({ ...prev, [name]: value })); }; const handleLogoChange = (e) => { const file = e.target.files[0]; if (file) { setLogoFile(file); setLogoPreview(URL.createObjectURL(file)); } }; const handleSubmit = async (e) => { e.preventDefault(); setIsSaving(true); await onSave({ ...formData, logo: logoFile }); setIsSaving(false); }; return ( <form onSubmit={handleSubmit} className="mt-6 bg-white p-6 rounded-lg shadow-md"> <div className="pb-4 mb-6 border-b border-gray-300"> <h2 className="text-xl font-semibold text-gray-700">Configurações do Restaurante</h2> </div> <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6"> <div className="space-y-6"> <div> <label htmlFor="nome" className="block text-sm font-medium text-gray-700">Nome do Restaurante</label> <input type="text" name="nome" id="nome" value={formData.nome} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"/> </div> <div> <label htmlFor="quantidade_mesas" className="block text-sm font-medium text-gray-700">Número Total de Mesas</label> <input type="number" name="quantidade_mesas" id="quantidade_mesas" value={formData.quantidade_mesas} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"/> <p className="mt-1 text-xs text-gray-500">O sistema irá criar ou apagar mesas para corresponder a este número.</p> </div> </div> <div className="space-y-6"> <div> <label htmlFor="cor_principal" className="block text-sm font-medium text-gray-700">Cor Principal</label> <div className="mt-1 flex items-center gap-3"> <input type="color" name="cor_principal" id="cor_principal" value={formData.cor_principal} onChange={handleChange} className="h-10 w-14 rounded-md border-gray-300 p-1"/> <span className="font-mono text-sm text-gray-600 bg-gray-100 px-2 py-1 rounded">{formData.cor_principal}</span> </div> </div> <div> <label className="block text-sm font-medium text-gray-700">Logo</label> <div className="mt-1 flex items-center space-x-4"> {logoPreview ? <img src={logoPreview} alt="Logo preview" className="h-16 w-16 rounded-full object-cover"/> : <div className="h-16 w-16 rounded-full bg-gray-100 flex items-center justify-center text-gray-400">?</div> } <input type="file" onChange={handleLogoChange} accept="image/*" className="text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 cursor-pointer"/> </div> </div> </div> </div> <div className="pt-6 mt-6 border-t border-gray-200 text-right"> <button type="submit" disabled={isSaving} className="bg-indigo-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-indigo-700 disabled:bg-indigo-300"> {isSaving ? 'A Guardar...' : 'Guardar Alterações'} </button> </div> </form> );
};
    ...
    <div>
      ...
      <div className="border-b border-gray-200">
        <nav className="-mb-px flex space-x-6" aria-label="Tabs">
          ...
          <button onClick={() => setActiveTab('mesas')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'mesas' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
            Gerir Mesas
          </button>
      ...
      {activeTab === 'mesas' && <GerirMesas mesas={mesas} restaurante={restaurante} />}
      
      {activeTab === 'restaurante' && <GerirRestaurante restaurante={restaurante} onSave={handleSaveRestaurante} />}
