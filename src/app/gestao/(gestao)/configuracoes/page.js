...
import { MODO_LABEL, isModoPrePago } from '@/lib/gestaoNav';
...
const GerirRestaurante = ({ restaurante, onSave }) => {
  const [formData, setFormData] = useState({ nome: '', cor_principal: '#4F46E5' });
...
export default function ConfiguracoesPage() {
  ...
  const carregarDados = async () => {
    ...
    try {
      const [categoriasData, itensData, restauranteData] = await Promise.all([
