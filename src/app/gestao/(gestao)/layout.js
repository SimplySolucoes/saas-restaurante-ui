...
export default function GestaoLayout({ children }) {
  ...
  if (!isAuth) {
    return <div className="flex h-screen w-full items-center justify-center">A verificar autenticação...</div>;
  }
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-gray-800 text-white flex justify-around p-2 border-t border-gray-700">
        {navLinks.map(link => (
          <Link key={link.href} href={link.href} className={`flex flex-col items-center justify-center p-2 rounded-md transition-colors w-full ${
