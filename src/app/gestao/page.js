...
const AnalyticsHomeIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="mb-3 h-8 w-8 text-indigo-500">
...
export default function GestaoHomePage() {
  ...
  const nav = navFlagsFromRestaurante(restaurante || user?.restaurante);

  const restauranteNome = user?.restaurante?.nome || "Gestor";
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-gray-800 mb-2">
        Bem-vindo, {restauranteNome}!
      </h1>
      <p className="text-gray-600 mb-8">Este é o seu painel de controlo. O que gostaria de fazer hoje?</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {nav.mesaAtiva && (
          <Link href="/gestao/mesa-ativa" className="block p-6 bg-white rounded-lg shadow-md hover:shadow-lg hover:scale-105 transition-transform">
            ...
            <p className="text-sm text-gray-600 mt-1">Monitorize os pedidos em tempo real e gira as contas.</p>
          </Link>
        )}
        {nav.novoPedido && (
        ...
        {nav.pedidos && (
          <Link href="/gestao/pedidos" className="block p-6 bg-white rounded-lg shadow-md hover:shadow-lg hover:scale-105 transition-transform">
            <PedidoIcon />
            <h2 className="font-bold text-xl text-gray-800">Pedidos</h2>
            <p className="text-sm text-gray-600 mt-1">Lista de pedidos por código (em evolução).</p>
          </Link>
        )}
        {isAdmin && nav.analytics && (
          <Link href="/gestao/analytics" className="block p-6 bg-white rounded-lg shadow-md hover:shadow-lg hover:scale-105 transition-transform">
            <AnalyticsHomeIcon />
            <h2 className="font-bold text-xl text-gray-800">Analytics</h2>
            <p className="text-sm text-gray-600 mt-1">Faturamento, produtos e histórico de pedidos.</p>
          </Link>
        )}
        {isAdmin && (
          <Link href="/gestao/configuracoes" className="block p-6 bg-white rounded-lg shadow-md hover:shadow-lg hover:scale-105 transition-transform">
            ...
            <p className="text-sm text-gray-600 mt-1">Edite o seu cardápio, mesas e as informações do restaurante.</p>
          </Link>
        )}
      </div>
    </div>
