...
export default async function LoginPage({ params }) {
  ...
  // Se o restaurante não for encontrado, exibe uma página de erro.
  if (!branding) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-100 text-center">
        <div>
          ...
          <p className="mt-2 text-gray-600">O endereço que você acedeu não parece ser válido.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex min-h-screen items-stretch bg-gray-100">
      ...
      >
        <div className="w-full max-w-sm text-center">
          {branding.logo ? (
            ...
            <div className="mx-auto h-24 w-24 rounded-full bg-white/20 flex items-center justify-center">
              {/* Ícone genérico */}
            </div>
          )}
      ...
      <div className="flex flex-1 md:w-3/5 flex-col items-center justify-center p-4">
        <div className="w-full max-w-md">
          ...
          {/* Passamos a cor principal para o formulário para usar nos botões e focos */}
